...
| `DFKV_RDMA_DEPTH` | `1` | **保持 1** | 单连接在途请求数。**延迟隐藏、非吞吐旋钮**——server 单连接 serve loop 串行，实测 PUT/GET 吞吐对 depth 全 flat（1≈8≈16≈32）。吞吐杠杆是多连接 fan-out（`batch_concurrency`）+ 少而大的 key，见 [datapath-perf-notes.md](datapath-perf-notes.md) |
...
# DFKV_RDMA_DEPTH 保持默认 1（§1.2：depth-flat）
```
...
- **`DFKV_RDMA_DEPTH` — 保持默认 1**（§1.2：PUT/GET 都 depth-flat）。
